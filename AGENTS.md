...
| Frontend | React Router 8 + Express (`frontend/`) | 3000 | Admin UI, auth, proxies WebDAV/API to backend |
...
├── entrypoint.sh            # Starts frontend, runs DB maintenance, then starts backend
...
**Container startup:** `entrypoint.sh` starts the frontend first so it can serve database-maintenance progress, runs backend EF migrations via `--db-migration`, then starts the backend and waits for `/health`. `Program.cs` registers controllers, WebDAV (`DatabaseStore`), and hosted services.
...
**Testing — run locally only what CI does not cover.** PR CI (`ci.yml`) always
runs the frontend and quality-ratchet lanes; path classification adds backend,
library, architecture, PostgreSQL, native-platform, and HTTP-contract lanes
when their inputs change. Do not repeat CI-covered checks locally unless a
focused run helps diagnose a failure. The local runs CI excludes are:
